...
from cherrymusicserver.db.connect import AbstractConnector, BoundConnector
...
class Updater(object):
    ...
    def needed(self):
        ...
        log.debug('%s update check: version=[%s] target=[%s]', self.name, version, target)
    ...
    def run(self):
        ...
        log.info('%r: updating database schema', self.name)
        log.debug('from version %r to %r', self._version, self._target)
    ...
    def reset(self):
        ...
        log.info('%s: resetting database', self.name)
        log.debug('version: %s', version)
        if None is version:
            log.debug('nothing to reset.')
            return
    ...
    def _setversion(self, value, conn=None):
        ...
        log.debug('{0}: set version to {1}'.format(self.name, value))
    ...
    def _init_meta(self):
        ...
        with self.db.transaction() as txn:
            ...
            if content and self._version is None:
                log.debug('%s: unversioned content found: %r', self.name, content)
    ...
    def _update_to_version(self, vnum):
        log.debug('updating database %r to version %d', self.name, vnum)
        with self.db.transaction() as txn:
            txn.isolation_level = "EXCLUSIVE"
